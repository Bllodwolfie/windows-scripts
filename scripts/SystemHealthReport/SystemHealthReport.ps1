...
[void]$html.AppendLine('    --radius-md: 16px; --radius-lg: 20px;')
[void]$html.AppendLine('    --glass-blur-lg: blur(10px) saturate(1.4);')
...
[void]$html.AppendLine('@media (prefers-reduced-motion: reduce) {')
[void]$html.AppendLine('  *, *::before, *::after {')
[void]$html.AppendLine('    animation-duration: 0.01ms !important;')
[void]$html.AppendLine('    animation-iteration-count: 1 !important;')
[void]$html.AppendLine('    transition-duration: 0.01ms !important;')
[void]$html.AppendLine('    scroll-behavior: auto !important;')
[void]$html.AppendLine('  }')
[void]$html.AppendLine('}')
[void]$html.AppendLine("body { font-family: 'Segoe UI', -apple-system, sans-serif; background: var(--base); color: var(--text); padding: 20px; }")
[void]$html.AppendLine('')
[void]$html.AppendLine('.header { display: flex; align-items: center; padding: 12px 20px; margin-bottom: 28px; background: color-mix(in srgb, var(--mantle) 80%, transparent); backdrop-filter: blur(8px) saturate(1.4); box-shadow: inset 0 1px 0 rgba(255,255,255,0.08); border-radius: var(--radius-lg); position: sticky; top: 0; z-index: 100; }')
# White inset glass-edge highlight reads as a faint rim on dark; on the light Latte
# surface it needs higher opacity to stay visible, so boost it per theme.
[void]$html.AppendLine('[data-theme="latte"] .header { box-shadow: inset 0 1px 0 rgba(255,255,255,0.5); }')
[void]$html.AppendLine('[data-theme="latte"] .sidebar { box-shadow: inset 1px 0 0 rgba(255,255,255,0.5); }')
[void]$html.AppendLine('.header-body { flex: 1; text-align: center; }')
[void]$html.AppendLine('h1 { font-size: 34px; font-weight: 700; color: var(--text); }')
[void]$html.AppendLine('.subtitle { color: var(--subtext0); font-size: 15px; font-weight: 400; margin-top: 6px; }')
...
[void]$html.AppendLine('')
[void]$html.AppendLine('h2 { font-size: 20px; font-weight: 600; margin: 32px 0 14px; padding-bottom: 8px; border-bottom: 2px solid var(--surface0); color: var(--text); display: flex; align-items: center; gap: 8px; }')
...
[void]$html.AppendLine('.card { background: var(--mantle); border-radius: var(--radius-md); padding: 16px 18px; transition: transform 0.25s cubic-bezier(0.4,0,0.2,1), box-shadow 0.25s cubic-bezier(0.4,0,0.2,1), backdrop-filter 0.25s cubic-bezier(0.4,0,0.2,1); cursor: default; position: relative; overflow: hidden; }')
[void]$html.AppendLine('.card:hover { transform: translateY(-4px); box-shadow: 0 8px 24px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.08); backdrop-filter: blur(4px) saturate(1.2); }')
...
[void]$html.AppendLine('.card-label { display: block; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.6px; color: var(--subtext0); margin-bottom: 4px; }')
[void]$html.AppendLine('.card-value { display: block; font-size: 17px; font-weight: 400; color: var(--text); letter-spacing: -0.3px; }')
[void]$html.AppendLine('.card-sub { display: block; font-size: 15px; font-weight: 400; color: var(--overlay1); margin-top: 2px; }')
[void]$html.AppendLine('')
...
[void]$html.AppendLine('')
[void]$html.AppendLine('table { width: 100%; border-collapse: collapse; background: var(--mantle); border-radius: var(--radius-md); overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }')
[void]$html.AppendLine('th { background: var(--surface0); color: var(--subtext1); font-size: 13px; text-transform: uppercase; letter-spacing: 0.6px; padding: 11px 14px; text-align: left; font-weight: 600; }')
[void]$html.AppendLine('td { padding: 9px 14px; border-bottom: 1px solid var(--surface0); font-size: 17px; font-weight: 400; vertical-align: top; }')
...
[void]$html.AppendLine('')
[void]$html.AppendLine('.footer { margin-top: 40px; padding: 20px; border-top: 1px solid var(--surface0); font-size: 12px; font-weight: 400; color: var(--overlay0); text-align: center; }')
[void]$html.AppendLine('')
...
[void]$html.AppendLine('    font-size: 34px; font-weight: 700; color: var(--text);')
...
[void]$html.AppendLine('    width: 44px; height: 44px; border-radius: 50%;')
...
[void]$html.AppendLine('.header .theme-toggle:focus-visible,')
[void]$html.AppendLine('.sidebar-close:focus-visible,')
[void]$html.AppendLine('.sidebar a:focus-visible {')
...
[void]$html.AppendLine('')
[void]$html.AppendLine('/* High-contrast preference: strengthen borders/dividers for both themes */')
[void]$html.AppendLine('@media (prefers-contrast: more) {')
[void]$html.AppendLine('    .card { border: 1px solid var(--surface1); }')
[void]$html.AppendLine('    th { background: var(--surface1); }')
[void]$html.AppendLine('    th, td { border-bottom: 1px solid var(--surface2); }')
[void]$html.AppendLine('}')
[void]$html.AppendLine('')
...
[void]$html.AppendLine('    backdrop-filter: var(--glass-blur-lg);')
[void]$html.AppendLine('    box-shadow: inset 1px 0 0 rgba(255,255,255,0.08);')
[void]$html.AppendLine('    border-right: 1px solid var(--surface0);')
[void]$html.AppendLine('    border-radius: 0 var(--radius-lg) var(--radius-lg) 0;')
...
[void]$html.AppendLine('    width: 36px; height: 36px; border-radius: 50%;')
...
[void]$html.AppendLine('            <svg viewBox="0 0 24 24" width="18" height="18"><line x1="18" y1="6" x2="6" y2="18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="6" y1="6" x2="18" y2="18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>')
